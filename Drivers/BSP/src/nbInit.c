...
/**
	* @brief  AT_CFUN : run/get/set UE Functionality
  * @param  Instruction parameter
  * @retval NB_TaskStatus
  */
NB_TaskStatus nb_cfun_run(const char* param)
{
	NBTask[_AT_CFUN].try_num = 3;
	NBTask[_AT_CFUN].set(NULL);
	...
	while(NBTask[_AT_CFUN].try_num--)
	{
		if(nb_at_send(&NBTask[_AT_CFUN])== NB_CMD_SUCC)
		{
			if(sys.fdr_flag == 1)
			{
				NBTask[_AT_CFUN].nb_cmd_status = NB_CFUN_CLOSE;
			}
			else if(sys.fdr_flag == 0)
			{
				NBTask[_AT_CFUN].nb_cmd_status = NB_CFUN_OPEN;
			}
		}
		else
		{
...
{
	if(sys.fdr_flag == 1)
		NBTask[_AT_CFUN].ATSendStr = AT CFUN "=0" NEWLINE;
	else
...
/**
	* @brief  AT_NBAND : Run/Set/Get frequency band
  * @param  Instruction parameter
  * @retval NB_TaskStatus
  */
		...
		{			
			...
			if(pch!=NULL)
			{
				...
				while(*pch != '\n')
					printf("%c",*pch++);
				printf("\n");
				NBTask[_AT_NBAND].nb_cmd_status = NB_CMD_SUCC;
			}
			break;
		}
		else
		{
			NBTask[_AT_NBAND].nb_cmd_status = NB_CMD_OFF;
		}
	}
...
{
	if(sys.fdr_flag == 1)
		NBTask[_AT_NBAND].ATSendStr = AT NBAND "=1,3,5,8,20,28"NEWLINE;
	else
		NBTask[_AT_NBAND].ATSendStr = AT NBAND "?"NEWLINE;
...
case _AT_CFUN:
			if(NBTask[_AT_CFUN].run(NULL) == NB_CFUN_OPEN)
			{
				*task=_AT_NCONFIG;
			}
			else if(NBTask[_AT_CFUN].run(NULL) == NB_CFUN_CLOSE)
			{
				*task=_AT_NBAND;
			}
			else 
			{
				at_state = _AT_ERROR;
				NBTask[_AT_CFUN].run(AT CFUN "=1" NEWLINE);
...
case _AT_NBAND:
			if(NBTask[_AT_NBAND].run(NULL) == NB_CMD_SUCC)
				*task=_AT_CSQ;
			else 
				at_state = _AT_ERROR;
			break;
